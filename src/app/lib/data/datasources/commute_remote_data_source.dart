...
/// Remote FastAPI datasource for /plan and /replan.
class CommuteRemoteDataSource {
  CommuteRemoteDataSource({required String baseUrl, http.Client? httpClient})
  ...
  Future<PlanResponseModel> plan(Map<String, dynamic> body) async {
    ...
    final parsed = PlanResponseModel.fromJson(map);
  ...
  Future<ReplanResponseModel> replan(Map<String, dynamic> body) async {
    final res = await _http
        ...
        .timeout(const Duration(seconds: 120));
    final map = _decodeMap(res, allowErrorBodies: true);
    final parsed = ReplanResponseModel.fromJson(map);
    if (res.statusCode >= 400 && parsed.error == null) {
      throw ApiException(
        statusCode: res.statusCode,
        message: _httpErrorMessage(res, map),
